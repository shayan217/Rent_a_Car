...
if (strlen($_SESSION['login']) == 0) {
  ...
  if (isset($_POST['updateprofile'])) {
    ...
    if ($_FILES['profile_picture']['error'] === UPLOAD_ERR_OK) {
      $targetDir = "img/profile_images/";
      $tempName = $_FILES['profile_picture']['tmp_name'];
      $imageName = $_SESSION['login'] . "_" . time() . "_" . $_FILES['profile_picture']['name'];
      $targetPath = $targetDir . $imageName;
      // Move the uploaded image to the target directory
      if (move_uploaded_file($tempName, $targetPath)) {
        // Image upload successful, update the profile picture in the database
        $sqlUpdateImage = "UPDATE tblusers SET ProfilePicture = :profilePicture WHERE EmailId = :email";
        $queryUpdateImage = $dbh->prepare($sqlUpdateImage);
        $queryUpdateImage->bindParam(':profilePicture', $targetPath, PDO::PARAM_STR);
        $queryUpdateImage->bindParam(':email', $email, PDO::PARAM_STR);
        $queryUpdateImage->execute();
      } else {
        // Image upload failed
        $error = "Failed to upload the profile picture. Please try again.";
      }
    }
  }
